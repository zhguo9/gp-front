<template>
  <div>
    ...
    <!-- 右侧图片 -->
    <div class="right">
      <img src="../../assets/pretrain.png" alt="图片" style="width: 600px; height: auto;">
    </div>
  </div>
  ...
  methods: {
    ...
    async processFile(file) {
      try {
        ...
        const response = await fetch(`http://localhost:8080/api/process1?fileName=${encodeURIComponent(file.name)}`, {
          method: 'POST',
          // 可以根据后端要求设置请求头和其他参数
        });
        ...
        //得到原位点
        const response1 = await fetch(`http://localhost:8080/api/getoriginal1?fileName=${encodeURIComponent(file.name)}`, {
          method: 'POST',
          // 可以根据后端要求设置请求头和其他参数
        });
